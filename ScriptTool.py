...
class ScriptTool:
    ...
    def assertCrop1(self, percentRect, expectImage, rgbThresh=(0, 0, 0), display=True):
        ...
        if(display):
            ...
            for p in range(1, 11):
                xP = p / 10.0
                drawObject.line([w * xP, 0, w * xP, h], fill="green", width=2)
            for p in range(1, 11):
                yP = p / 10.0
                drawObject.line([0, h * yP, w, h * yP], fill="green", width=2)
            #screen.show()
            x1, y1, x2, y2 = (
                int(w * percentRect[0]), int(h * percentRect[1]), int(w * percentRect[2]), int(h * percentRect[3]))
            drawObject.line((x1, y1, x2, y1), fill="red", width=1)
            drawObject.line((x2, y1, x2, y2), fill="red", width=1)
            drawObject.line((x1, y2, x2, y2), fill="red", width=1)
            drawObject.line((x1, y1, x1, y2), fill="red", width=1)
            screen.show()
